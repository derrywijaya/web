...
def query_all_items(container, account_number):
    print('\nQuerying for an  Item by Partition Key\n')

    # Including the partition key value of account_number in the WHERE filter results in a more efficient query
    items = list(container.query_items(
        query="SELECT * FROM r WHERE r.partitionKey=false"
        # parameters=[
        #     { "name":"@account_number", "value": account_number }
        # ]
    ))
    ...
    for item in items:
        ...
        try:
            folder = item.get("email")
            folder = os.path.join('uploadedData',folder)
            if not os.path.exists(folder):
                os.makedirs(folder)
        
            fileId = os.path.join(folder, item.get("id")+".json")
            with open(fileId, 'w', encoding='utf-8') as json_file:
                json.dump(item, json_file, ensure_ascii=False)
        except :
            print('\nrun_sample has caught an error. {0}'.format(item.get("email")))

    print('Item queried by Partition Key {0}'.format(items[0].get("email")))
    # print(items)
...
def run_sample():
    ...
    try:
        ...
        query_all_items(container, 'Account1')
